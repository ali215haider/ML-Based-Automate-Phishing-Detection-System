...
def download_report():
    ...
    # Title
    title = Paragraph(
        f"PhishShield Pro - Scan History Report for {current_user.username}",
        styles['Title'])
    story.append(title)
    ...
    if scans:
        ...
        for scan in scans:
            data.append([
                ...
                scan.scan_type.capitalize(), scan.content[:50] +
                '...' if len(scan.content) > 50 else scan.content,
    ...
    return send_file(
        ...
        as_attachment=True,
        download_name=
        f'phishield_report_{current_user.username}_{datetime.now().strftime("%Y%m%d")}.pdf',
...
def report_phishing():
    ...
    flash('Thank you for reporting! We will investigate this content.',
          'success')
    return redirect(url_for('profile'))
